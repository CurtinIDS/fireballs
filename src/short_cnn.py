...
def ls_time(path):
    time = lambda myfile: os.stat(os.path.join(path, myfile)).st_mtime
    return sorted(os.listdir(path), key=time)[0]
...
def tile(filename,w,h):
    ...
    while (j<int(Y/h)):  #Divide vertically into Y/h tiles
        ...
        i=0
        w_x=0
        while (i<int(X/w)): #Divide horizontally into X/w tiles
            i+=1
            if (i==int(X/w)):
                w_x=(X-w)
